...
export async function loadAboutContent() {
  ...
  const response = await fetch(
    `${supabaseUrl}/rest/v1/pages?url_path=eq.${encodeURIComponent("/about/")}&status=eq.published&select=content,meta_title,meta_description,canonical_url,og_title,og_description,og_image,noindex,schema_type,schema_data`,
    {
      headers: {
        apikey: supabaseKey,
